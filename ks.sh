...
CSV="нет"          # выгружать ли таблицы проверок и ресурсов в CSV
...
do_report_xlsx() {
  if ! "$PY" -c 'import openpyxl' >/dev/null 2>&1; then
    printf 'Для .xlsx нужен модуль openpyxl, его нет:  pip install openpyxl\n'
    local answer
    ask answer 'Собрать вместо этого таблицы в CSV (они без модулей)? [Д/н]: ' || return
    case "$answer" in
      н|Н|n|N) return;;
      *) CSV="да"; do_overview; return;;
    esac
  fi
...
menu() {
  ...
  printf 'Отчёт: %s\n' "$(label_of "$REPORT")"
  printf 'Имена людей: %s   Строк на экране: %s   Таблицы в CSV: %s\n' "$NAMES" "$TOP" "$CSV"
  cat <<'MENU'
------------------------------------------------------------------
